...
def run_game(row_group):
    global xcords, ycords, game_exit, exit, level_exit, clicks, end, start, screen, click_loss, timed_loss, cur_puzz
...
class ClickableSprite(pygame.sprite.Sprite):
    ...
    def update(self, events):
        ...
        global game_exit, level_exit, oprandom, optimed, opclicks, modepuzzle
        for event in events:
            if event.type == pygame.MOUSEBUTTONUP:
                if self.rect.collidepoint(event.pos):
                    ...
                    if x == 49 and y == 150: expand_cords(7,3), screen_size(), game(oprandom, optimed, opclicks) # Normal
